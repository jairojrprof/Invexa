...
async function queryProvider(ticker, kind, fetcher, token) {
  ...
  try {
    const r = await fetcher(url.href, { headers: { Authorization: `Bearer ${token}` }, signal: AbortSignal.timeout(10000) })
    if (r.status === 429) throw fail(503, 'PROVIDER_LIMITED', 'O provedor atingiu o limite de consultas. Tente novamente mais tarde.', 60)
...
export function createMarketHandler(kind, { fetcher = globalThis.fetch, cache, now = Date.now, token = () => process.env.BRAPI_TOKEN?.trim() } = {}) {
  ...
  async function getAsset(ticker) {
    ...
    const promise = (async () => {
      ...
      const payload = await queryProvider(ticker, kind, fetcher, token())
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      const entries = await Promise.all(tickers.map(getAsset))
