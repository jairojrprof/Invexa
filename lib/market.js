...
export const TTL = { quote: 3600, dividends: 21600 }
export const PROVIDER_INTERVAL_MS = 1200
...
async function queryProvider(ticker, kind, fetcher, token, deadline) {
  ...
  try {
    ...
    if (r.status === 429) {
      ...
      throw fail(503, 'PROVIDER_LIMITED', 'A BRAPI limitou as consultas do serviço. Aguarde antes de atualizar novamente.', Math.max(1, Math.min(Number.isFinite(seconds) && seconds > 0 ? seconds : 300, 86400)))
    }
...
export function createMarketHandler(kind, { fetcher = globalThis.fetch, cache, now = Date.now, sleep = (ms, signal) => delay(ms, undefined, { signal }), token = () => process.env.BRAPI_TOKEN?.trim() } = {}) {
  // Coalesce consultas simultâneas na instância; não substitui o limitador.
  const inFlight = new Map()
  const memory = new Map()
  const cooldownKey = 'invexa:xxidrfxkykpwacvpdllv:market:provider-cooldown:v1'
  let queue = Promise.resolve(), nextStart = 0, blockedUntil = 0
  function remember(key, entry) {
    memory.delete(key)
    memory.set(key, entry)
    if (memory.size > 512) memory.delete(memory.keys().next().value)
  }
  async function pacedQuery(ticker, deadline) {
    const task = queue.then(async () => {
      try {
        const shared = await cache?.get(cooldownKey)
        if (Number.isFinite(shared?.until)) blockedUntil = Math.max(blockedUntil, shared.until)
      } catch { /* conserva a pausa local se o cache estiver indisponível */ }
      if (now() < blockedUntil) throw fail(503, 'PROVIDER_LIMITED', 'A BRAPI limitou as consultas do serviço. Aguarde antes de atualizar novamente.', Math.ceil((blockedUntil - now()) / 1000))
      try {
        if (nextStart > now()) await sleep(nextStart - now(), deadline)
        if (deadline.aborted) throw new DOMException('deadline', 'AbortError')
      } catch { throw fail(504, 'PROVIDER_TIMEOUT', 'A consulta demorou para responder. Tente novamente mais tarde.') }
      nextStart = now() + PROVIDER_INTERVAL_MS
      try { return await queryProvider(ticker, kind, fetcher, token(), deadline) }
      catch (error) {
        if (error.code === 'PROVIDER_LIMITED') {
          blockedUntil = now() + error.retryAfter * 1000
          try { await cache?.set(cooldownKey, { until: blockedUntil }, { ttl: error.retryAfter }) } catch { /* pausa local permanece */ }
        }
        throw error
      }
    })
  ...
  async function getAsset(ticker, deadline) {
    ...
    const promise = (async () => {
      ...
      if (!entry || now() - entry.fetchedAt >= TTL[kind] * 1000) {
        try { entry = await cache?.get(key) || entry } catch { /* cache é uma otimização */ }
      }
      const age = now() - entry?.fetchedAt
      if (entry?.payload && age >= 0 && age < TTL[kind] * 1000) { remember(key, entry); return entry }
      if (deadline.aborted) throw fail(504, 'PROVIDER_TIMEOUT', 'O provedor demorou para responder. Tente novamente.')
      const payload = await pacedQuery(ticker, deadline)
  ...
  return async function handler(req, res) {
    ...
    try {
      ...
      // Clientes antigos também deixam de consumir a BRAPI para um recurso pago.
      if (kind === 'dividends') throw fail(403, 'DIVIDENDS_UNAVAILABLE', 'Os dados de dividendos estão indisponíveis nesta versão. Isso não significa que você não recebeu proventos.')
